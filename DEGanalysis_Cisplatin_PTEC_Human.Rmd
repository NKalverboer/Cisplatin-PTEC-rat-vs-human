---
title: "BOO2024 - DEGanalysis_Cisplatin_PTEC_Human"
...
```{r}
metadata = fread(file.path(input_dir, "20240605_filtered_metadata_processedHuman2.csv"))
...
```{r}
countdata_raw = fread(file.path(input_dir, "20240605_filtered_countdata_raw_processedHuman2.csv"))
...
deseq_results %>% 
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, size=10, family = "noto_sans", face = "bold"), axis.text.y = element_text(family= "noto_sans", face= "bold", size = 10), plot.title = element_text(family = "noto_sans", size = 15), axis.title.x = element_text(family= "noto_sans", size = 12),
        axis.title.y = element_text(family= "noto_sans", size = 12)
        ) +
  labs(x = "Treatmend condition", y = "n", title = "Differentially expressed genes")
```
...
order <- c(   "HPPTEC_CSP_8hr_2.5",   "HPPTEC_CSP_8hr_10",   
 "HPPTEC_CSP_8hr_50",    "HPPTEC_CSP_24hr_2.5", 
 "HPPTEC_CSP_24hr_10",   "HPPTEC_CSP_24hr_50",   
"HPPTEC_CSP_72hr_2.5",  "HPPTEC_CSP_72hr_10",   "HPPTEC_CSP_72hr_50")
...
#     drop_na() %>%
    mutate(MEAN_ID_TREATMENT = factor(MEAN_ID_TREATMENT, levels = order), class = if_else(abs(log2FoldChange) >
        log2FC_threshold & padj < padj_threshold, "significant", "not significant"))
...
plot <- ggplot(data = red, mapping = aes(x = log2FoldChange, y = -log10(padj), color = class)) + 
  ...
  geom_point(size = 0.5) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, face = "bold", family = "noto_sans", size = 10), 
  ...
  labs(title = "Volcano plot human DEG")
...
deseq_results = deseq_results %>% mutate(MEAN_ID_TREATMENT = factor(MEAN_ID_TREATMENT, levels = order))
...
pca_data = pca_data %>% mutate(MEAN_ID = factor(MEAN_ID, levels = order))
...
```{r}
write_delim(expression_long_TXG2, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_expression_long_TXG_upload_humanORA2.txt")))
```

# Save output
```{r}
write_rds(deseq_results, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_DESeq2_results_unformatted_human.rds"))) 
write_delim(expression_long_TXG, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_expression_long_TXG_upload_human.txt")))
